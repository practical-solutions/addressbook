...
class helper_plugin_addressbook_db extends DokuWiki_Plugin
{
    ...
    /**
     * Modified for addressbook plugin by Gero Gothe
     * 
    ...
    protected function init()
    {
        ...
        if (!$this->sqlite->init('addressbook', DOKU_PLUGIN . 'addressbook/db/')) { # Changed plugin name
            if (defined('DOKU_UNITTEST')) {
    ...
    public function getDB($throw=true)
    {
        ...
        if(!$this->sqlite && $throw) {
            throw new \Exception('The addressbook plugin requires the sqlite plugin. Please install and enable it.');
        }
        return $this->sqlite;
    }
